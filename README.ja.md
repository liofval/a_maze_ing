...
| メンバー | 役割 |
|----------|------|
| kaztakam | 設定パーサー、出力フォーマッター、バリデーションロジック、テスト |
| monoda | 迷路生成アルゴリズム、ビジュアル表示、アニメーション、プロジェクト統合 |

### 計画とその変遷

**当初の計画:**

1. データモデルの定義（Direction, Cell, Maze）
2. Recursive Backtracker アルゴリズムの実装
3. 設定パーサーと出力フォーマッターの構築
4. インタラクティブメニュー付きターミナル表示の追加
5. 再利用可能な `mazegen` ライブラリとしてパッケージ化

**変遷:**

- Strategy パターンの拡張性を示すために、2つ目の生成手法として Kruskal アルゴリズムを追加
- Python ジェネレータ (`yield`) を使ったアニメーション機能を追加。これにはアルゴリズムを `generate_steps()` で中間状態を公開するようリファクタリングする必要があった
- エッジケース（1x1 迷路、巨大な迷路、範囲外座標）のテスト後に入力バリデーションを強化

### うまくいったこと

- `mazegen/`（ライブラリ）と `app/`（アプリケーション）を早期に分離したことで、コードベースがクリーンになり、パッケージ化が容易になった
- Strategy パターンにより Kruskal の追加が容易 — 新規ファイル1つと辞書エントリ1行
- `Maze.remove_wall()` レベルで壁の整合性を強制することで、バグの一分類全体を排除
- Direction に `IntFlag` を使用したことで、hex 出力に変換ロジックが不要

### 改善できること

- ターミナルレンダラーは生の ANSI エスケープコードを使用している。`curses` や `blessed` のようなライブラリを使えば、端末間の互換性が向上する
- 不完全迷路生成のエッジケースに対するテストカバレッジを拡大できる
- アニメーション速度がハードコードされている。設定ファイルで変更可能にすればユーザビリティが向上する
...
| mypy (--strict) | 静的型チェック |
| pytest | ユニットテスト |
| setuptools + build | パッケージビルド (.whl, .tar.gz) |
| Claude (AI) | アーキテクチャ設計、コード生成、ドキュメント |
